...
func (n *nftablesRunner) createDummyPostroutingChains() error {
	...
	for _, table := range n.getNATTables() {
		...
		if err := deleteTableIfExists(n.conn, table.Proto, tsDummyTableName); err != nil {
			return fmt.Errorf("delete %q table: %w", tsDummyTableName, err)
		}
	}
	return nil
}
